...
const fetchRoomsAPI = (): Promise<Room[]> => {
...
const fetchAmenitiesAPI = (): Promise<Amenity[]> => {
  return new Promise((resolve) => {
    setTimeout(() => {
      resolve([
...
function* fetchRoomsSaga() {
  try {
    const rooms: Room[] = yield call(fetchRoomsAPI);
    yield put({
...
function* fetchAmenitiesSaga() {
  try {
    const amenities: Amenity[] = yield call(fetchAmenitiesAPI);
    yield put({
